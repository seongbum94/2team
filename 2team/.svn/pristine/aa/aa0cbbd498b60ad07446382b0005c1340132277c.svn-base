...
public class ApplyDropController {
    ...
    void btnOkClicked(ActionEvent event) {		
    	...
    	if(!tableView.getSelectionModel().isEmpty()) {	// 선택한 값이 있다면...
    		...
    		ButtonType bt = AlertController.confirm("자퇴 승인 처리할 학생: " + applyOList.get(index).get("stu_name"),"해당 학생의 자퇴 승인을 하시겠습니까?");
    		...
    		if(bt.getText().equals("OK")) {
    			...
    			if(cnt>1) {	// 업데이트 성공!!	
    				AlertController.info("처리결과", "자퇴 승인 처리가 완료됐습니다.");
    				// 테이블 뷰에 다시 출력하기
    	    		setTableViewData();
    				//changeTableView(index);
    			}else {
    				AlertController.warnning("처리결과", "자퇴 처리 실패..전산팀에 문의하세요.");
    				
    			}
    		}

    	}else {		// 선택한 값이 없는 경우
    		AlertController.warnning("선택오류", "선택한 학생이 없습니다.");
    	}
    }
    ...
    void btnRejectClicked(ActionEvent event) {	 
    	...
    	if(!tableView.getSelectionModel().isEmpty()) {	// 선택한 값이 있다면...
    		...
    		ButtonType bt = AlertController.confirm("자퇴신청 반려 처리할 학생: " + applyOList.get(index).get("stu_name"), "해당 학생의 자퇴신청 반려을 하시겠습니까?");
